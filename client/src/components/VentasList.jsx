...
const VentaList = () => {
    ...
    useEffect(()=>{
        axios.get('http://localhost:8000/api/venta')
            .then(res=>{
                setVenta(res.data);
            });
    },[venta]);
    ...
    return (
        ...
        <table className="table table-striped table-hover table-bordered">
            ...
            <tbody>
                { venta.map( (venta, index) => <tr key={index} >
                <td>{ venta.name + " " + venta.lstname }</td>
                <td>{ venta.lat + ", " + venta.lng }</td>
                <td>
                    <DeleteVenta id={id} />
                </td>
                </tr> ) }
